...
ROLLOUT_RESULT_PATH =  Path(__file__).resolve().parent / "rollout_result"
...
class TinyPhysicsSimulator:
  ...
  def reset(self) -> None:
    ...
    #seedcode = self.data_path + chr(np.random.randint(ord('%'),ord('z')))
    #seed = int(md5(seedcode.encode()).hexdigest(), 16) % 10**4
  ...
  def rollout(self) -> Dict[str, float]:
    if self.debug:
      ...
      mngr.window.setGeometry(50,100,1640, 845)
...
def collectSaveSimRun(result, prefix, path = 'results'):
    df = result[1]
    df['steer_command'][CONTROL_START_IDX:]= np.array(result[2][CONTROL_START_IDX:])
    #pdb.set_trace()
    #df['target_lataccel']= result[3]
    df['actual_lataccel']= result[4]
    filepath = path / f'{prefix}.csv'
    df.to_csv(filepath)
...
def run_rollout(data_path, controller_type, model_path, debug=False, COLLECT=False):
  ...
  if not COLLECT:
    ...
    cost = sim.rollout()
  #pdb.set_trace()
    return cost, sim.data, sim.action_history, sim.target_lataccel_history, sim.current_lataccel_history


def run_rollout_ctrl(data_path, controller_type, model_path, debug, controller_params ):
  tinyphysicsmodel = TinyPhysicsModel(model_path, debug=debug)
  #pdb.set_trace()
  controller = importlib.import_module(f'controllers.{controller_type}').Controller()
...
if __name__ == "__main__":
  ...
  if args.collect and not ROLLOUT_RESULT_PATH.exists():
    ROLLOUT_RESULT_PATH.mkdir(parents=True, exist_ok=True)
  ...
  if data_path.is_file():
    ...
    run_rollout_partial = partial(run_rollout, controller_type=args.controller, model_path=args.model_path, debug=False, COLLECT=args.collect)
    files = sorted(data_path.iterdir())[:args.num_segs]
    results = process_map(run_rollout_partial, files, max_workers=MAX_PROCESSES, chunksize=10)
    if args.collect:
        for i, result in enumerate(results):
            collectSaveSimRun(result, prefix=args.controller+f"_run{i}", path = ROLLOUT_RESULT_PATH) 
